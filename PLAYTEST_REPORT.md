...
Implementation update after this task: the default Pixi battlefield now sizes
its host through a small client-only fit-to-stage helper instead of the previous
width/aspect-ratio guess. The helper measures the available board stage and
chooses the largest 700:650 rectangle that fits both width and height, while
leaving Pixi's internal layout constants, coordinate math, rules, and simulator
untouched. Normal `/` also no longer spends player-facing cockpit space on idle
`Mode: Inspect` or `Mode: Loadout` diagnostic panels; those remain available
under `?debug=1`, and actionable placement/loadout status still appears when a
card is selected or placement mode is active. Browser smoke now asserts the
Pixi host has nonzero dimensions, stays inside the measured board stage, keeps
...
Manual layout follow-up after that pass: the wider center column improved the
default route, but it also exposed the fixed-size Pixi board. The renderer still
scales a 700 x 650 root into its host using the smaller width/height ratio, and
the default host was height-clamped, so the actual board stayed roughly the same
size while the host grew into empty dark side gutters. Implementation update
after this task: the Battlefield panel now uses a compact `default-pixi-cockpit`
with a capped board area and a desktop sidecar for existing board/loadout/
...
animate more fluidly while keeping cell-based rules; Commander upgrades still
feel bland and may need build-defining perk-tree-like choices; Board Charge,
Sources, and Combat Charge/sec needed stronger teaching; and one later fight
ended in a draw often enough that draw frequency should be reviewed later.
...
Implementation update after this task: the default route's `Last Recorded
Combat` panel now starts with a compact `Combat recap`. It summarizes the
player-facing result (`Victory`, `Draw`, or `Defeat`), damage dealt/taken, gold
...
- The tightened default Pixi route has now had a fresh 10-minute manual pass at
  ...
  Battlefield cockpit with a fit-to-stage 700:650 Pixi host, a desktop sidecar for
